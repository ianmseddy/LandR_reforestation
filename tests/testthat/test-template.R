...
#    `functionName` is a function's name in your module (e.g., `LandR_reforestationEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Git/LandR_reforestation/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LandR_reforestation")
  path <- list(modulePath = "C:/Ian/Git",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    LandR_reforestation = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LandR_reforestationEvent1", envir = .GlobalEnv)) {
    simOutput <- LandR_reforestationEvent1(mySim)
  } else {
  ...
  if (exists("LandR_reforestationEvent2", envir = .GlobalEnv)) {
    simOutput <- LandR_reforestationEvent2(mySim)
  } else {
